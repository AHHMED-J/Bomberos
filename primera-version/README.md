# Parte digital · Bomberos de Ensenada — Primera versión
...
## Estructura
    ...
    suite-investigacion/    las 6 interfaces de "Instrumentos digitales para la
                             investigación de usuarios" — ver su propio README

`docs/` es el único de estas carpetas que GitHub Pages sirve; `investigacion/`,
`app/` y `suite-investigacion/` viven en el repo pero no en el sitio público
(`suite-investigacion/` se abre local, no necesita estar publicada).
...
- **Regenerar la hoja:** `python3 build.py` (desde `primera-version/`). Escribe
  ...
  genera una versión de computadora. Cada lámina sale dos veces: en celular
  (`.screen`, la misma versión que `screens/` y la portada) y en tablet (con
  la clase de más `.screen--tablet`, sólo para comparar dentro de esta hoja).
...
se ve sin estilos, revisa que todas las rutas sean relativas (`css/…`,
`screens/…`), nunca absolutas (`/css/…`), porque el sitio vive en un
subdirectorio.
